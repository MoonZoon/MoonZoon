...
fn click_me_button<'a>() -> Row<'a> {
    let title: &'static Mutable<_> = &Lazy::new(|| Mutable::new("Click me!".to_owned()));
    let click_count: &'static Mutable<_> = &Lazy::new(|| Mutable::new(0_u32));
    Row::new()
        .item(Button::new()
            .label(title.signal_ref())
            .on_press(move || {
...
fn test_counters<'a>() -> Row<'a> {
    Row::new()
        .item("Test counters")
        .item(Counter::new()
            .value(super::test_counter_value().signal())
...
fn column_counter<'a>() -> Row<'a> {
    Row::new()
        .item("Columns:")
        .item(Counter::new()
            .value(super::column_count().signal())
