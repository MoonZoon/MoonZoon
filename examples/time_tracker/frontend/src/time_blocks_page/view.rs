...
fn client(client: Arc<super::Client>) -> impl Element {
    Column::new()
        ...
        .item(add_entity_button(
            "Add Time Block",
            clone!((client) move || super::add_time_block(&client)),
        ))
...
fn time_blocks(client: Arc<super::Client>) -> impl Element {
    Column::new().s(Spacing::new(20)).items_signal_vec(
        client
            .time_blocks
            .signal_vec_cloned()
            .map(move |tb| time_block(client.clone(), tb)),
    )
}
...
fn add_invoice_button(time_block: Arc<super::TimeBlock>) -> impl Element {
    El::new()
        ...
        .child(add_entity_button("Add Invoice", move || {
            super::add_invoice(&time_block)
        }))
}

fn timeblock_name_duration_and_delete_button(
    client: Arc<super::Client>,
    time_block: Arc<super::TimeBlock>,
) -> impl Element {
    ...
    Row::new()
        ...
        .item(delete_entity_button(move || {
            super::delete_time_block(&client, id)
        }))
}
...
fn time_block_duration_input(time_block: Arc<super::TimeBlock>) -> impl Element {
    ...
    let (text_duration, text_duration_signal) =
        Mutable::new_and_signal_cloned(time_block.duration.get().num_hours().to_string());
    ...
    TextInput::new()
        ...
        .s(Background::new().color_signal(
            is_valid_signal
                .map_bool_signal(|| theme::background_0(), || theme::background_invalid()),
        ))
        .s(Borders::new().bottom_signal(theme::border_1().map(|color| Border::new().color(color))))
        .s(Padding::all(5))
        ...
        .on_change(move |text| {
            ...
            if let Ok(hours) = hours {
                time_block
                    .duration
                    .set_neq(Duration::seconds((hours * 3600.) as i64).into());
                debounced_set_duration.set(Some(Timer::once(
                    app::DEBOUNCE_MS,
                    clone!((time_block) move || {
...
fn status_button(
    label: &str,
    represent_status: TimeBlockStatus,
    time_block: Arc<super::TimeBlock>,
) -> impl Element {
    ...
    Button::new()
        ...
        .s(Background::new().color_signal(
            hovered_or_active
                .signal()
                .map_true_signal(|| theme::background_3()),
        ))
        .s(Font::new().color_signal(
            hovered_or_active
                .signal()
                .map_bool_signal(|| theme::font_3(), || theme::font_0()),
        ))
...
fn invoice(time_block: Arc<super::TimeBlock>, invoice: Arc<super::Invoice>) -> impl Element {
    El::new().s(Padding::all(10)).child(
        Column::new()
            .s(Padding::new().left(10))
            .s(Background::new().color_signal(theme::background_1()))
            .s(RoundedCorners::all(40 / 2))
            .item(invoice_custom_id_and_delete_button(
                time_block.clone(),
                invoice.clone(),
            ))
...
fn invoice_custom_id_and_delete_button(
    time_block: Arc<super::TimeBlock>,
    invoice: Arc<super::Invoice>,
) -> impl Element {
    Row::new().item(invoice_custom_id(invoice)).item(
        El::new()
            .s(Align::new().right())
            .s(Padding::new().bottom(5))
            .child(delete_entity_button(move || {
                super::delete_invoice(&time_block)
            })),
    )
}
...
fn invoice_custom_id(invoice: Arc<super::Invoice>) -> impl Element {
    ...
    El::new().child(
        TextInput::new()
            ...
            .s(Background::new().color_signal(theme::transparent()))
            .s(Borders::new()
                .bottom_signal(theme::border_1().map(|color| Border::new().color(color))))
            .s(Padding::all(5))
            .placeholder(
                Placeholder::new("Invoice custom ID").s(Font::new().color_signal(theme::font_0())),
            )
...
fn invoice_url_and_link_button(invoice: Arc<super::Invoice>) -> impl Element {
    Row::new().item(invoice_url(invoice.clone())).item(
        El::new()
            .s(Align::new().right())
            .s(Padding::new().top(5))
...
fn invoice_url(invoice: Arc<super::Invoice>) -> impl Element {
    ...
    El::new().child(
        TextInput::new()
            ...
            .s(Padding::all(5))
            .placeholder(
                Placeholder::new("Invoice URL").s(Font::new().color_signal(theme::font_0())),
            )
