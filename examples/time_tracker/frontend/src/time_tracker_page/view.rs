...
fn start_stop_button(project: Arc<super::Project>) -> impl Element {
    ...
    Button::new()
        ...
        .s(Font::new().color_signal(
            has_active_entry
                .signal()
                .map_bool_signal(|| theme::font_4(), || theme::font_3()),
        ))
...
fn time_entries(project: Arc<super::Project>) -> impl Element {
    Column::new().s(Spacing::new(20)).items_signal_vec(
        project
            .time_entries
            .signal_vec_cloned()
            .map(move |t| time_entry(project.clone(), t.clone())),
    )
}
...
fn time_entry(project: Arc<super::Project>, time_entry: Arc<super::TimeEntry>) -> impl Element {
    ...
    let is_active_updater =
        Task::start_droppable(time_entry.stopped.signal().for_each_sync(move |stopped| {
    ...
    Column::new()
        .s(Background::new().color_signal(
            is_active
                .signal()
                .map_bool_signal(|| theme::background_4(), || theme::background_1()),
        ))
        ...
        .item(time_entry_name_and_delete_button(
            project,
            time_entry.clone(),
            is_active.clone(),
        ))
...
fn time_entry_name_and_delete_button(
    ...
    Row::new()
        ...
        .item(delete_entity_button(
            move || super::delete_time_entry(&project, id),
            is_active,
        ))
}
...
fn time_entry_name(
    ...
    El::new().s(Padding::all(10)).child(
        TextInput::new()
            ...
            .s(Background::new().color_signal(theme::transparent()))
            .s(
                Borders::new().bottom_signal(is_active.signal().map_bool_signal(
    ...
    Button::new()
        ...
        .s(Font::new()
            .color_signal(
                is_active
                    .signal()
                    .map_bool_signal(|| theme::font_1(), || theme::font_3()),
            )
...
fn date_time_part_input(
    ...
    let text_updater =
        Task::start_droppable(number.for_each_sync(clone!((valid, focused) move |number| {
            if not(focused.get()) {
                valid.set_neq(true);
                if max_chars == Some(2) {
    ...
    TextInput::new()
        ...
        .s(Background::new().color_signal(
            valid_signal.map_bool_signal(|| theme::transparent(), || theme::background_invalid()),
        ))
        ...
        .read_only_signal(
            is_active
                .signal()
                .map_bool(move || read_only_when_active, || false),
        )
        .on_change(move |text| valid.set_neq(on_change(text).is_some()))
}
