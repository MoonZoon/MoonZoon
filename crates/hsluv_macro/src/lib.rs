...
pub fn hsluv(input: TokenStream) -> TokenStream {
    ...
    for arg in &args {
        match arg {
            Lit::Float(lit_float) => match lit_float.to_string().parse() {
            ...
            Lit::Int(lit_int) => match lit_int.to_string().parse() {
                Ok(number) => numbers.push(number),
                Err(error) => {
            ...
            lit => {
                return Error::new_spanned(lit, "only float and int literals expected")
                    .into_compile_error()
                    .into()
            }
        }
    ...
    let [h, s, l, a] = match numbers.as_slice() {
        ...
        [h, s, l, a] => [h, s, l, a],
        _ => {
            return Error::new_spanned(args, "3 (hsl) or 4 (hsla) arguments expected")
                .into_compile_error()
                .into()
        }
    };
    ...
    if *h < 0. || *h > 360. {
        return Error::new_spanned(&args[0], "h (hue) value in the range 0..=360 expected")
            .into_compile_error()
            .into();
    }
    if *s < 0. || *s > 100. {
        return Error::new_spanned(
            &args[1],
            "s (saturation) value in the range 0..=100 expected",
        )
        .into_compile_error()
        .into();
    }
    if *l < 0. || *l > 100. {
        return Error::new_spanned(
            &args[2],
            "l (lightness) value in the range 0..=100 expected",
        )
        .into_compile_error()
        .into();
    }
    if *a < 0. || *a > 100. {
        return Error::new_spanned(&args[3], "a (alpha channel) in the range 0..=100 expected")
            .into_compile_error()
            .into();
    }
